...
size_t abb_vectorizar_inorden(abb_t* abb, void** vector, size_t tamaño)
{
    if (!abb || !vector)
        return 0;
    size_t cantidad_iterados = 0;
    return !recorrido_inorden(abb->raiz, asignar_elementos_en_vector, &vector, &cantidad_iterados, tamaño) ? cantidad_iterados : abb->nodos;
}
...
size_t abb_vectorizar_preorden(abb_t* abb, void** vector, size_t tamaño)
{
    if (!abb || !vector)
        return 0;
    size_t cantidad_iterados = 0;
    return !recorrido_preorden(abb->raiz, asignar_elementos_en_vector, &vector, &cantidad_iterados, tamaño) ? cantidad_iterados : abb->nodos;
}
...
{   
    if (!abb || !vector)
        return 0;
    size_t cantidad_iterados = 0;
    return !recorrido_postorden(abb->raiz, asignar_elementos_en_vector, &vector, &cantidad_iterados, tamaño) ? cantidad_iterados : abb->nodos;
}