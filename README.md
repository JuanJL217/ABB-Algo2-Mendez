...
`Funciones de Vectorización`: `O(n)`, ya que, aunque haya un valor `m`, que es el tamaño del vector, el peor caso siempre será recorrer todo el arbol, y sabiendo que asginar valores es `O(1)`, por eso la complejidad algoritmica de las funciones de vectorización quedan en `O(n)`.